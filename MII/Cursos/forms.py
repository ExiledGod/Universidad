from Cursos.models import Detalle_curso, Lista_alumnos, Cursos, AvanceCurricular, Malla_curricular
...
class Detalle_curso_form(forms.ModelForm):
	class Meta:
		...
		widgets = {
			'anio_cursado': forms.NumberInput(attrs={'class':'form-control'}),
			'semestre': forms.NumberInput(attrs={'class':'form-control'}),
			'paralelo': forms.NumberInput(attrs={'class':'form-control'}),
			'inscritos': forms.NumberInput(attrs={'class': 'form-control'}),
...
class Cursos_form(forms.ModelForm):
	class Meta:
		...
		labels = {
			...
			'creditos': 'CRÉDITOS: ',
			'tipo': 'TIPO DE CURSO: ',
			'curso_en_malla': 'CURSO CORRESPONDIENTE A MALLA: ',
		...
		widgets = {
			...
			'curso_en_malla': forms.Select(attrs={'class': 'form-control'}),
			'descripcion': forms.TextInput(attrs={'class':'form-control'})
		}
...
class Malla_curricular_form(forms.ModelForm):
	class Meta:
		...
		labels = {
			...
			'malla': 'Seleccione la malla actual:',
		}
		widgets = {
			'curso': forms.Select(attrs={'class':'form-control'}),
			'malla': forms.Select(attrs={'class':'form-control'}),
		}