...
from .models import Alumno, Becas, Cursos_homologados, Postulacion, Pagos, DireccionAlumno, Cobros
...
@login_required()
def becasAgregarDatos(request, pk):
    alumno = get_object_or_404(Alumno, pk=pk)
    beca = BecasForm(request.POST or None)
    if beca.is_valid():
        agregar_beca = beca.save(commit=False) #todavía no lo guardo
        agregar_beca.alumno = alumno #le agrego el id del alumno
        agregar_beca.save() #lo guardo
        messages.success(request, 'Se han agregado correctamente los datos de la beca')
        return redirect('alumnos_home', pk)

    return render(request, 'Alumno/crear_beca.html', {'form': beca})
...
class eliminar_alumno(LoginRequiredMixin, DeleteView):
    model = Alumno
    template_name = 'Alumno/eliminar_alumno.html'
    form_class = AlumnoForm
    success_url = reverse_lazy('mantenedor_alumno')
...
def postulacionAgregarDatos(request, pk):
    ...
    if postulacion.is_valid():
        ...
        messages.success(request, 'Se han agregado correctamente los datos de postulación')
        return redirect('ficha_academica', pk)

    return render(request, 'Alumno/agregar_datos_postulacion.html', {'form': postulacion})

@login_required()
def datos_postulacion(request, pk):
    objeto1 = Alumno.objects.get(id=pk)
    objeto2 = Postulacion.objects.get(alumno_id=pk)

    return render(request, 'Alumno/datos_postulacion.html', {'alumno': objeto1, 'postulacion': objeto2})

@login_required()
def editar_postulacion(request, pk): 
    postulacion = Postulacion.objects.get(id=pk)
...
@login_required()
def busqueda_ficha_alumno(request):
    ...
    if query:
        ...
        results = []
    return render_to_response('Alumno/busqueda_ficha.html', {'results': results, 'query': query})


@login_required()
def busqueda_alumno_cobro(request):
    query = request.GET.get('q', '')
    if query:
        qset = (
            Q(nombre__icontains=query) | 
        ...
        results = []
    return render_to_response('Alumno/busqueda_alumno_cobro.html', {'results': results, 'query': query})


@login_required()
def cobros_alumno(request, pk):

    objeto1 = Alumno.objects.filter(id=pk)
    objeto2 = Cobros.objects.filter(alumno_id=pk)
    objeto3 = Pagos.objects.filter(alumno_id=pk)

    return render(request, 'Alumno/cobros.html', {'alumno': objeto1, 'cobros': objeto2, 'pagos': objeto3})  



@login_required()
def pagos_alumno(request, pk):

    objeto1 = Alumno.objects.filter(id=pk)
    objeto2 = Cobros.objects.filter(alumno_id=pk)
    objeto3 = Pagos.objects.filter(alumno_id=pk)

    return render(request, 'Alumno/pagos.html', {'alumno': objeto1, 'cobros': objeto2, 'pagos': objeto3})  


class info_becas(LoginRequiredMixin, ListView):
    model = Alumno
    template_name = 'Alumno/informacion_sobre_becas.html'
    form_class = AlumnoForm
    success_url = reverse_lazy('informacion_sobre_becas')


class crear_direccion_alumno(LoginRequiredMixin, CreateView):
    model = DireccionAlumno
    template_name = 'Alumno/crear_direccion_alumno.html'
    form_class = DireccionForm
    success_url = reverse_lazy('mantenedor_alumno')

class alumnos_becados(LoginRequiredMixin, ListView):
    model = Alumno
    template_name = 'Alumno/alumnos_becados.html'
    form_class = AlumnoForm
    paginate_by = 10 
    success_url = reverse_lazy('alumnos_becados')


@login_required()
def busqueda_alumno_pago(request):
    query = request.GET.get('q', '')
    if query:
        qset = (
            Q(nombre__icontains=query) | 
        ...
        results = []
    return render_to_response('Alumno/busqueda_alumno_pago.html', {'results': results, 'query': query})


@login_required()
def busqueda_alumno_deuda(request):
    query = request.GET.get('q', '')
    if query:
        qset = (
            Q(nombre__icontains=query) | 
        ...
        results = []
    return render_to_response('Alumno/deudas.html', {'results': results, 'query': query})

@login_required()
def deudas_alumno(request, pk):

    objeto1 = Alumno.objects.filter(id=pk)
    objeto2 = Cobros.objects.filter(alumno_id=pk)
    objeto3 = Pagos.objects.filter(alumno_id=pk)

    return render(request, 'Alumno/detalle_deudas.html', {'alumno': objeto1, 'cobros': objeto2, 'pagos': objeto3}) 
...
def plantilla_alumnos(request):
    ...
    if query:
        ...
        results = []
    return render_to_response('Mantenedores/plantilla_alumno.html', {'results': results, 'query': query})


@login_required()
def plantilla_docente(request):
    query = request.GET.get('q', '')
    if query:
        qset = (
            Q(nombre__icontains=query) | 
        ...
        results = []
    return render_to_response('Mantenedores/plantilla_docente.html', {'results': results, 'query': query})


@login_required()
def plantilla_cursos(request):
    query = request.GET.get('q', '')
    if query:
        qset = (
            Q(nombre__icontains=query) | 
            Q(sigla__icontains=query) |
            Q(creditos__icontains=query)
        )
        results = Cursos.objects.filter(qset).distinct()
    else:
        results = []
    return render_to_response('Mantenedores/plantilla_cursos.html', {'results': results, 'query': query})





#FUNCIONES DE LOS MANTENEDORES DE ALUMNO

@login_required()
def plantilla_principal(request):
    return render(request, 'Mantenedores/plantilla_principal.html')


@login_required()
def mantenedor_alumnos_home(request, pk):

    objeto1 = Alumno.objects.filter(id=pk)
    objeto2 = DireccionAlumno.objects.filter(alumno_id=pk)
    objeto3 = Postulacion.objects.filter(alumno_id=pk)

    return render(request, 'Mantenedores/alumno_home.html', {'alumno': objeto1, 'direccion': objeto2, 'postulacion': objeto3})
...
def acta_interna(request):
    ...
    objeto1 = Alumno.objects.get(id=10)   
